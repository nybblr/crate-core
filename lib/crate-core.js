...
CrateCore.prototype.insert = function(character, index){
    ...
    this.broadcast.send(JSON.stringify(new MInsertOperation(ei, id._e)));
    return ei;
};
...
CrateCore.prototype.remove = function(index){
    ...
    this.broadcast.send(JSON.stringify(new MRemoveOperation(i, id._e)));
    return i;
};
...
CrateCore.prototype.caretMoved = function(range){
    this.sequence._c += 1;
    var id = {_e:this.sequence._s, _c: this.sequence._c } // (TODO) fix uglyness
    this.broadcast.send(JSON.stringify(new MCaretMovedOperation(range, id._e)));
    return range;
};
